    ...
    public static class Node<E> {
        ...
        private void EatOrReproduce(boolean isGoingRight){
            ...
            if (this.GetElement() != null) {
                if (this.GetElement().getClass() == Bear.class) {
                    isBear = true;
                }
                Animal myClass = ((Animal) this.GetElement());
                if (isGoingRight) { //I'm going right
                    Animal nextClass = ((Animal) this.GetNext().GetElement());
                    if (myClass.getClass() == nextClass.getClass()) {
                        //Then the Animals are of same type and reproduce
                        Reproduce(isGoingRight);
                    } else { //They are not the same type
                        if (isBear) { //Am I a Bear?
                            this.GetNext().SetElement(null); //Eat the Fish
                        } else { //I am a Fish
                            this.SetElement(null);//The Bear eats me
                        }
                    }
                } else { //I'm going left
                    Animal prevClass = ((Animal) this.GetPrevious().GetElement());
                    if (myClass.getClass() == prevClass.getClass()) {
                        //Then the Animals are of the same type and reproduce
                        Reproduce(isGoingRight);
                    } else { //They are not the same type
                        if (isBear) { //Am I a Bear?
                            this.GetPrevious().SetElement(null); //Eat the Fish
                        } else { //I am a Fish
        ...
        private void Reproduce(boolean isGoingRight){
            Animal myType = ((Animal)this.GetElement());
            if (isGoingRight){ //Add between this and the node to the right
                Node newAnimal = new Node(myType, this.GetNext(), this);
                this.GetNext().SetPrevious(newAnimal);
                this.SetNext(newAnimal);
            } else { //Add between this and the node to the left
                Node newAnimal = new Node(myType, this, this.GetPrevious());
                this.GetPrevious().SetNext(newAnimal);
                this.SetPrevious(newAnimal);
            }
        }
        private boolean DidICollide(boolean isGoingRight){
            if (isGoingRight){
                ...
                if (this.GetNext().GetElement() != null){
                    return true; //I hit another Animal
                }
